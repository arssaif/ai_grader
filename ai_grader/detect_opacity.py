...
def opacity(adder,newName,user_id):
    ...
    #print('\n\n',device)
    model = attempt_load('static\models\opacity/best.pt')  # load FP32 model
    ...
    for i, det in enumerate(pred):
        a=len(det)# detections per image
        #print(det)
        if det is None or len(det) <=0 :
            title='No Opacity'
        if det is not None and len(det):
            title=' Opacity Detected'
    ...
    if not os.path.exists('static/opacity/User' + str(user_id)):
