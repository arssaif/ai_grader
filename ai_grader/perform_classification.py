...
def model_working(adder):
    ...
    with strategy.scope():
        my_model = tf.keras.models.load_model(
            r'static/models/disease_classification/best_model.h5')
        my_model.load_weights(r'static/models/disease_classification/best_model_weights.h5')
...
def predict(p1, newName, user_id):
    ...
    for p in ax.patches:
        ...
        if width < 10 and width>0.5:
            ax.annotate(f'{width:00.2f}'+'%' ,xy=(width+5, bottom+height/2), ha='center', va='center')
        if width > 10:
             ax.annotate(f'{width:00.2f}'+'%' ,xy=(left+width/2, bottom+height/2), ha='center', va='center')
